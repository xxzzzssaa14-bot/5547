...
window.saveEditTransaction = async function() {
    ...
    trans.totalCost = newTotal; // تنبيه: تعديل الإجمالي يدوياً سيثبت المبلغ المتراكم
    trans.paid = newPaid;
...
window.renderAlerts = function() {
    ...
    if(!hasAlerts) {
        list.innerHTML = '<p style="text-align:center; color:#fff; margin-top:20px; font-weight:bold; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">لا توجد تنبيهات حالياً.</p>';
    }
}
